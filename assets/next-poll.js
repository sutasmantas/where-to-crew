(function(){
  ...
  function card(t){
    return '<article class="trip-card" data-trip="'+esc(t.id)+'" data-budget="'+t.budget+'">'+
      ...
      '<details class="trip-details"><summary>Activity plan, exact inclusions &amp; booking</summary><div class="detail-content"><a class="booking-link" href="'+url(t.sources[0][1])+'" target="_blank" rel="noopener">Open the main provider / booking page ↗</a><h4>Realistic activity plan</h4><ol class="activity-plan">'+t.plan.map(function(p){return '<li>'+esc(p)+'</li>';}).join('')+'</ol><h4>Core versus choices</h4><div class="cost-scope"><p><b>Core includes:</b> '+esc(t.coreIncludes)+'</p><p><b>Only if chosen:</b> '+esc(t.optional)+'</p></div><h4>How the price was built</h4><p class="basis">'+esc(t.basis)+'</p><h4>Evidence and booking links</h4><div class="source-list">'+t.sources.map(function(s){return '<a href="'+url(s[1])+'" target="_blank" rel="noopener">'+esc(s[0])+' ↗</a>';}).join('')+'</div></div></details></div></article>';
  }

  grid.innerHTML = trips.map(card).join('');
  document.getElementById('candidateCount').textContent = trips.length+' choices';
  var words={0:'No',1:'One',2:'Two',3:'Three',4:'Four',5:'Five',6:'Six',7:'Seven',8:'Eight',9:'Nine',10:'Ten',11:'Eleven',12:'Twelve',13:'Thirteen',14:'Fourteen',15:'Fifteen'};
  var heroCount=document.getElementById('heroCandidateCount'), compareCount=document.getElementById('compareCount');
  if(heroCount) heroCount.textContent=words[trips.length]||trips.length;
  if(compareCount) compareCount.textContent='Compare all '+trips.length+' ↓';
